...
### How do I update DClaw Lease?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp lease --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Lease without Kubernetes?

Yes, for local development:

```bash
cd dclaw-lease/frontend && npm run dev
cd dclaw-lease/backend && uvicorn dclaw_lease.main:app --reload
```
